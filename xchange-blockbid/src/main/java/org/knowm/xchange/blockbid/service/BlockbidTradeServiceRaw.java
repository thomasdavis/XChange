...
public class BlockbidTradeServiceRaw extends BlockbidBaseService {
  static final Logger logger = Logger.getLogger("BlockbidBaseService");

  public BlockbidTradeServiceRaw(Exchange exchange) {
    super(exchange);
  }

  public BlockbidOrderResult makeBlockbidMarketOrder(
      CurrencyPair currencyPair, String side, BigDecimal originalAmount) throws IOException {
    ...
    BlockbidOrdersRequest request = new BlockbidOrdersRequest(market, blockbidOrders);
    try {
      List<BlockbidOrderResult> order =
          blockbid.makeMarketOrder(
              apiKey, signatureCreator, exchange.getNonceFactory().createValue(), request);
      return order.get(0);
  ...
  public BlockbidLimitOrderResult placeBlockbidLimitOrder(
      CurrencyPair currencyPair, String side, BigDecimal originalAmount, BigDecimal price)
      throws IOException {
    String market = currencyPair.toString().replace("/", "").toLowerCase();
    ...
    try {
      List<BlockbidLimitOrderResult> order =
          blockbid.placeLimitOrder(
              apiKey, signatureCreator, exchange.getNonceFactory().createValue(), request);
      return order.get(0);
  ...
  public boolean cancelBlockbidLimitOrder(String uuid) throws IOException {
    BlockbidLimitOrderResult cancelledOrder =
        blockbid.cancelOrder(
            apiKey, signatureCreator, exchange.getNonceFactory().createValue(), uuid);
    return true;
  }

  public List<BlockbidTradeResult> getBlockbidUserTrades(CurrencyPair currencyPair)
      throws IOException {
    String market = currencyPair.toString().replace("/", "").toLowerCase();
    List<BlockbidTradeResult> marketTradesResult =
        blockbid.getUserTrades(
            apiKey, signatureCreator, exchange.getNonceFactory().createValue(), market);
    return marketTradesResult;
  }
}
